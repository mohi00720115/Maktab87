...
/*
    val imageListDC = arrayListOf<ImageDataClass>(
    ...
    fun like(position: Int) {
        listLikes.add(imageListDC[position])
        imageListDC[position].isFavorite = true
    }

    fun disLike(position: Int) {
        listLikes.remove(imageListDC[position])
        imageListDC[position].isFavorite = false
    }

    fun likeOrDislike(position: Int) {
        when {
            imageListDC[position].isFavorite -> {
                disLike(position)
                return
            }
            else -> {
